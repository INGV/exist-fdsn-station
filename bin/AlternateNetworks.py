...
def add_virtual_networks(station_filename, station, dictionary,network_filename='AlternateNetwork.xml'):
...
def build_alternate_network_file(url='http://webservices.ingv.it/ingvws/virtualnetwork/1/codes',filename='AlternateNetwork.xml'):
    r = requests.get(url)
...
def build_alternate_network_lists(filename='AlternateNetwork.xml'):
    ...
    for child in root.iter('*'):
        ...
        if child.tag == '{https://raw.githubusercontent.com/FDSN/StationXML/master/fdsn-station.xsd}AlternateNetwork' :
            ...
            query=f'http://webservices.ingv.it/fdsnws/station/1/query?level=station&net={code}&format=xml&nodata=404'
            #debug print(query)
