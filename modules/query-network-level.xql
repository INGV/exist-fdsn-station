...
declare function local:main_text(){
        ...
        transform:transform(stationutil:query_network_main(), doc("network.xsl"), ())
};    
    

if (request:get-parameter("format","xml")="xml") 
    then stationutil:query_network_main()
    else if (request:get-parameter("format","text")="text") then local:main_text()
    else ()
